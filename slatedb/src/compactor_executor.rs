...
mod tests {
    ...
    /// that. On `FilterError`/`CreationError`, `compaction_worker::handle_finished`
    /// clears `ctx` and reschedules from key zero (see
    /// `test_worker_restarts_from_key_zero_on_filter_error`).
    ///
    ...
    async fn test_resumed_last_run_init_filter_fails_past_sentinel() {
        ...
        impl CompactionFilterSupplier for InitSentinelFilterSupplier {
            async fn create_compaction_filter(
                ...
                // Exhaustive bind: CompactionJobContext::is_resume is the
                // resume flag. This filter still runs on resume so the Init
                // error still reproduces. Vector's supplier fails
                // CreationError instead.
        ...
        async fn write_sst_entries(
            ...
            table_store
                .write_sst(&SsTableId::from(Ulid::new()), &encoded, Some(Bytes::new()))
                .await
                .unwrap()
        }
        ...
        // Control: empty output_ssts, last-run. The filter sees the sentinel
        // first and the compact succeeds. Discriminates "last-run Init filter"
        // from "resume past sentinel".
        ...
        // Resume: checkpoint last key is a posting (past the sentinel), injected
        // as subcompaction output_ssts — forced resume, not a flaky few-doc
        // compact→close.
    ...
    /// Supplier half: `is_resume` is visible at
    /// `create_compaction_filter`, so a last-run supplier can fail closed
    /// with CreationError instead of installing a fresh Init filter.
    #[cfg(feature = "compaction_filters")]
    #[tokio::test]
    async fn test_last_run_supplier_fails_closed_on_resume() {
        ...
        fresh
            .run_compaction(vec![fresh_input], true, None)
            .await
            .expect("fresh last-run must still run");
